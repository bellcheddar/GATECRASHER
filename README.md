...
- [x] **Short molecular dynamics.** Four 5 ns runs, one per primary structure, each carrying the paper's own claims as testable statements written before it started. CDK2, KRAS and FGFR support theirs and publish their trajectories; JAK1 does not and its trajectory is withheld, which is the rule doing its job rather than failing. The measurement that most repays reading is FGFR's designed hydrogen bond to Asn571, present in 65% of frames at a 3.17 Å median: real, and intermittent. Every run also publishes what it could not test
